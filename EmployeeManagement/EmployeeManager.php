...
class EmployeeManager
{
	...
	/**
	 * Constructor - loads existing employees from JSON
	 */
	 ...
	 */
	private function askForOptionalInput(string $_label, string|int $_current, callable $_validator, string $_error): string|int|false
	{
		$attempts = 0;
		while ($attempts < self::MAX_ATTEMPTS) {
	...
	private function createEmployee(): void
	{
		...
		if ($type_choice === null)
			return;
		if ($type_choice == 1) {
			$salary = trim(readline("Enter Monthly Salary: "));
			$benefits = trim(readline("Enter Benefits: "));
			$employee = new FullTimeEmployee(
				$employee_id,
				$values['setFirstName'], $values['setLastName'],
				$values['setDepartment'], (int) $values['setExperienceOfEmployee'],
				$values['setPhoneNumber'], $values['setEmailAddress'],
				$values['setAadharNumber'], $values['setPanNumber'],
				$values['setDateOfBirth'], $values['setNationality'],
				$values['setMaritalStatus'], 'Full Time',
				(float) $salary, $benefits
			);
		} else {
			$hourly_rate = trim(readline("Enter Hourly Rate: "));
			echo "\nSelect Shift Type:\n";
			echo "1. Morning\n";
			echo "2. Afternoon\n";
			echo "3. Evening\n";
			echo "4. Night\n";
			$shift_choice = $this->getValidChoice(1, 4);
			if ($shift_choice === null) return;
			$shift_types = [1 => 'Morning', 2 => 'Afternoon', 3 => 'Evening', 4 => 'Night'];
			$shift_type = $shift_types[$shift_choice];
			$employee = new PartTimeEmployee(
				...
				$values['setAadharNumber'], $values['setPanNumber'],
				$values['setDateOfBirth'], $values['setNationality'],
				$values['setMaritalStatus'], 'Part Time',
				(float) $hourly_rate, $shift_type
			);
		}
